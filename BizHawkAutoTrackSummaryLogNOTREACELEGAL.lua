...
area_battles,area_frames,area_menus,LocTimes,LocParty,LocationBinary,KIsToLocMap,LocToKisMap,BossBattles,BossTimes,BossParty = {},{},{},{},{},{},{},{},{},{},{}
KIBinary=0
...
local function checkKIs()
	...
	local words= {}
	for i=0, 1 do
		words[i]=mainmemory.read_u32_le(0x1514 + 4*i)
	end
	for i=0, 1 do
		LocB=words[i]
		...
		for j=0,31 do
			if(bit.band(LocNb,bit.lshift(1,j)) ~= 0) then
				...
				KiB=bit.band(mainmemory.read_u32_le(0x1500),0x1FFFF)
...
local function countTreasure()
	local tres,mem=0,0
	for i=0,15 do
		mem=mainmemory.read_u32_le(0x12A0+4*i)
		for j=0,31 do
...
local function myframe()
	local menu = mainmemory.read_u8(0x0500)
	if(started) then
		local temp = mainmemory.read_u32_le(0x1700)
		local area = bit.band(temp,0xFF)
		local mapID = bit.band(temp,0xFF00) + bit.rshift(bit.band(temp,0xFF0000),16)
		local battle = mainmemory.read_u8(0x0201)
		temp = mainmemory.read_u32_le(0x1704)
